// functional_async_event_handling.cpp
#include <iostream>
#include <future>
#include <vector>
#include <algorithm>
...
int main() {
    EventHandler functionalHandler = [](const std::string& event) {
        std::cout << "[Functional] Received event: " << event << " in thread " << std::this_thread::get_id() << std::endl;
        // Complex logic (e.g., data transformation, filtering) can be encapsulated here
    };
